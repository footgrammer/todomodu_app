...
class ActivityHistoryDatasourceImpl implements ActivityHistoryDatasource {
  ...
  @override
  Future<void> save(ActivityHistory activity) async {
    final dto = ActivityHistoryDto.fromEntity(activity);

    final json =
        dto.toJson()
          ..['createdAt'] = FieldValue.serverTimestamp(); // 서버 타임 강제 적용

    await _collection.doc(activity.id).set(json);
  }

  @override
  Future<List<ActivityHistory>> fetchByProjectId(String projectId) async {
    final snapshot =
        await _collection
